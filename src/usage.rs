...
pub fn record(r: &Value) {
    ...
    let c = conn();
    let _ = c.execute(
        "INSERT INTO requests (ts, model, source, account_head, stream, status, prompt_tokens, completion_tokens, latency_ms, error)
...
pub fn summary(hours: i64) -> Value {
    ...
    let mut q = c
        .prepare(
            "SELECT status, latency_ms, prompt_tokens, completion_tokens, model, error FROM requests WHERE ts > ?1",
        )
        .expect("q");
    let mut rows = q.query(rusqlite::params![since]).expect("q rows");
    let (mut total, mut errors, mut pt, mut ct) = (0i64, 0i64, 0i64, 0i64);
